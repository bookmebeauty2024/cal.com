...
export default abstract class BaseCalendarService implements Calendar {
  ...
  async getAvailability(params: GetAvailabilityParams): Promise<EventBusyDate[]> {
    ...
    /**
     * If no calendars are connected, skip external checks.
     */
    if (!selectedCalendars || selectedCalendars.length === 0) {
      this.log.debug("[BOOKME] No external calendars connected, skipping availability check.");
      return [];
    }
    ...
    try {
      ...
      const userId = this.getUserId(selectedCalendars);
      const userTimeZone = userId ? await this.getUserTimezoneFromDB(userId) : "UTC";
      ...
      objects.forEach((object) => {
        ...
        try {
          ...
          this.log.warn("[BOOKME] Failed to parse calendar object", e);
          return;
        }
        ...
        vevents.forEach((vevent) => {
          ...
          const start = dayjs(event.startDate.toJSDate()).toISOString();
          const end = dayjs(event.endDate.toJSDate()).toISOString();
       ...
       * Calendar failures should NOT break slot generation.
       */
      this.log.warn(
        "[BOOKME] External calendar availability failed — continuing without calendar conflicts",
        error
      );
